...
const Karte: FC = () => {
  ...
  return (
    <div className="flex items-end top-[72px] bottom-0 h-full">
      ...
      <div className="absolute w-56 h-[7rem] bg-sersa2 top-[5.5rem] left-4 px-1.5 pt-0.5 box-shadow z-20">
        <LayerWidget />
      </div>
      <div className='absolute top-[5.5rem] left-1/2 z-20'>
        <MapTopicWidget />
      </div>
      <div className={`relative flex ${displayHeight === 'middle' ? 'h-1/2 w-full' : 'h-2.5 w-full z-20'} items-end`}>
        <div className={`flex ${displayHeightInfra3d === 'full' ? 'w-2/5 h-4/5 z-20' : 'w-1/2 h-4/5 z-20'} items-end justify-start`}>
          <MapDiagrams />
        </div>
        <div className={`flex ${displayHeightInfra3d === 'full' ? 'w-3/5 h-full z-20' : 'w-1/2 h-4/5 z-20'} items-end justify-end`}>
          <Infra3d />
        </div>
        {displayHeightInfra3d !== 'full' &&
        <div className={`absolute flex left-1/2 ${displayHeight === 'middle' ? 'top-[calc(20%_-_0.75rem)]' : '-top-3'}`}>
          <CollapsibleBox displayHeight={displayHeight} setDisplayHeight={setDisplayHeight} Component={'Map'}/>
        </div>
        }
      </div>
    </div>
